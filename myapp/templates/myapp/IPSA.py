...
def buscar_ufinet_nni_BOGOTA_2_4(texto):
    if not texto:
        return "No encontrado"
    
    patron = r'(.*UFINET.*NNI.*2\.4.*)'
    match = re.search(patron, texto, re.IGNORECASE)
    if match:
        return True
    return False
...
def main(cfs):
    try:
        ...
        if texto is None:
            print(json.dumps({"error": "No se pudo leer el archivo."}))
            return
        ...
        if buscar_ufinet_nni_BOGOTA_2_4(texto):
            resultados['sw'] = "BOGTCLFXNN001"
            resultados['interface_sw'] = "TEN GIGA 0/0/0/3"
        ...
        if resultados['interface_pe_b'] == "No encontrado":
            resultados['interface_pe_b'] = ""
